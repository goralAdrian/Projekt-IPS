...
const emptyCart = async (req, res, type) => {
    ...
    if(type === 'json'){
        res.status(200).json({ message: 'Koszyk opr�niono', totalCartItems: 0 });
        return;
    }

    req.session.message = 'Koszyk opr�niono';
...
router.post('/product/addtocart', async (req, res, next) => {
    ...
    if(req.session.cartSubscription){
        return res.status(400).json({ message: 'Nie mo�na wi�cej wzi��� ksi��ek' });
    }
