...
router.get('/admin/user/edit/:id', restrict, async (req, res) => {
    ...
    if(user.userEmail !== req.session.user && req.session.isAdmin === false){
        ...
        req.session.message = 'Odmowa dost�pu';
        req.session.messageType = 'danger';
        res.redirect('/admin/users');
        return;
    }
...
router.get('/admin/user/delete/:id', restrict, async (req, res) => {
    ...
    if(req.session.userId === req.params.id){
        if(req.apiAuthenticated){
            res.status(400).json({ message: 'Nie mo�esz usun�� swojego konta' });
            return;
        }

        req.session.message = 'Nie mo�esz usun�� swojego konta.';
        req.session.messageType = 'danger';
        res.redirect('/admin/users');
        return;
    }
    ...
    try{
        ...
        if(req.apiAuthenticated){
            res.status(200).json({ message: 'Usuni�to u�ytkownika.' });
            return;
        }
        req.session.message = 'Usuni�to u�ytkownika.';
