...
const updateConfig = (fields) => {
    ...
    if(fs.existsSync(localSettingsFile)){
        ...
        try{
            ...
            console.log('Nie udalo sie zapisac lokalnych ustawien plikow', exception);
        }
    }
...
const newMenu = (req) => {
    ...
    .catch((err) => {
        console.log('Nie uda�o si� utworzy� menu', err);
        return false;
    });
};
...
const getEmailTemplate = (result) => {
    ...
    $('#paymentMessage').text('Dziekujemy za zakup. Do zobaczenia nastepnym razem!');
...
const getData = (req, page, query) => {
    ...
    .catch((err) => {
        throw new Error('Nie udalo sie pobrac ksiazek z bazy danych');
    });
};
...
const hooker = (order) => {
    ...
    .then((response) => {
        if(response.status === 200){
            console.info('Pomy�lnie zaksi�gowano transakcj�');
        }
    })
    .catch((err) => {
        console.log('B��d przy zaksi�gowaniu transakcji:', err);
    });
};
