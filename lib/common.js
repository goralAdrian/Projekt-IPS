...
const updateConfig = (fields) => {
    ...
    if(fs.existsSync(localSettingsFile)){
        ...
        try{
            ...
            console.log('Nie uda³o siê zapisaæ lokalnych ustawieñ plików', exception);
        }
    }
...
const newMenu = (req) => {
    ...
    .catch((err) => {
        console.log('Nie uda³o siê utworzyæ menu', err);
        return false;
    });
};
...
const getEmailTemplate = (result) => {
    ...
    $('#paymentMessage').text('Dziêkujemy za zakup. Do zobaczenia nastêpnym razem!');
...
const getData = (req, page, query) => {
    ...
    .catch((err) => {
        throw new Error('Nie uda³o siê pobraæ ksi¹¿ek z bazy danych');
    });
};
...
const hooker = (order) => {
    ...
    .then((response) => {
        if(response.status === 200){
            console.info('Pomyœlnie zaksiêgowano transakcjê');
        }
    })
    .catch((err) => {
        console.log('B³¹d przy zaksiêgowaniu transakcji:', err);
    });
};
