...
const indexCustomers = (app) => {
    ...
    return new Promise((resolve, reject) => {
        app.db.customers.find({}).toArray((err, customerList) => {
            ...
            if(process.env.NODE_ENV !== 'test'){
                console.log(colors.cyan('- Pomyœlnie zakoñczono zaindeksowanie u¿ytkownika'));
            }
            resolve();
        });
    });
...
const indexOrders = (app, cb) => {
    ...
    return new Promise((resolve, reject) => {
        app.db.orders.find({}).toArray((err, ordersList) => {
            if(err){
                console.error(colors.red('B³¹d podczas indeksowania ksi¹¿ek: ' + err));
                reject(err);
            }
            ...
            if(process.env.NODE_ENV !== 'test'){
                console.log(colors.cyan('- Indeksowanie zosta³o zakoñczone poprawnie'));
            }
            resolve();
        });
    });
...
const runIndexing = (app) => {
    if(process.env.NODE_ENV !== 'test'){
        console.info(colors.yellow('Konfiguracja indeksów..'));
    }
    ...
    .catch((err) => {
        console.info(colors.yellow('B³¹d podczas indeksowania indeksów: ', err));
        process.exit(2);
    });
};
