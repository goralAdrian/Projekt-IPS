...
const checkLogin = async (req, res, next) => {
    ...
    if(req.headers.apikey){
        try{
            ...
            if(!user){
                res.status(400).json({ message: 'Odnowa dostêpu' });
                return;
            }
...
const checkAccess = (req, res, next) => {
    ...
    if(req.session.isAdmin === false && routeCheck){
        if(routeCheck.response === 'redirect'){
            req.session.message = 'Brak autoryzacji. Proszê skontaktowaæ siê z administratorem.';
            req.session.messageType = 'danger';
            res.redirect('/admin');
            return;
        }
        if(routeCheck.response === 'json'){
            res.status(400).json({ message: 'Brak autoryzacji. Proszê skontaktowaæ siê z administratorem.' });
        }
    }else{
        next();
    }
